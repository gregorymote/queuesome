...
spotifyObject2 = spotipy.Spotify(auth=token)
deviceResults = spotifyObject2.devices()
...
while True:
    ...
    for x in range(0, int(players)):
        ...
        if choice == "0":
            ...
            z = 0
            #Extract album data
            albumResults = spotifyObject2.artist_albums(artistID)
    ...
    for x in range(len(trackSelectionList)): 
        ...
        if x+1 < len(trackSelectionList):
            spotifyObject2.next_track(deviceID)
        else: 
            spotifyObject2.pause_playback(deviceID)
