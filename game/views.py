...
def run_game(pid):
    ...
    while Party.objects.filter(pk=pid, active=True).first():
        ...
        if Party.objects.filter(pk=pid, active=True, state="assign").first() or (
            Party.objects.filter(pk=pid, active=True, state="choose_category").first()
            and not Users.objects.filter(
                turn="picking", party__pk=pid, active=True
            ).first()
        ):
            party = Party.objects.get(pk=pid)
            assign_leader(party)
        ...
        if (
            Party.objects.filter(pk=pid, active=True, state="pick_song").first()
            and not Users.objects.filter(
                hasPicked=False, party__pk=pid, active=True
            ).all()
        ):
            party = Party.objects.get(pk=pid)
        ...
        if Party.objects.filter(
            pk=pid, active=True, device_error=False, thread=False
        ).first() and Songs.objects.filter(
        ...
        if form.is_valid():
            ...
            if valid:
                ...
                remaining_users = Users.objects.filter(
                    party=party, active=True, turn="not_picked"
                ).all()
...
def pick_category(request, pid):
    ...
    libraries = (
        Library.objects.filter(pk__in=choices, visible=True)
        | Library.objects.filter(special=True, visible=True)
    ).order_by("order", "id")
    cats = []
...
def pick_song(request, pid):
    ...
    category = Category.objects.filter(party=party, roundNum=party.roundTotal).first()

    if request.method == "POST":
        form = searchForm(request.POST)
        if form.is_valid():
            result = form.cleaned_data["result"]
            picked_song = Songs.objects.filter(user=user, category=category).first()
            if picked_song:
                user.hasPicked = True
                user.save()
                ...
                search = Searches.objects.filter(
                    uri=result, user=user, party=party
                ).first()
                song = Songs(
                    name=search.name,
                    title=search.title,
    ...
    context = {"form": form, "category": category, "invalid": invalid, "party": party}
...
def update_search(request):
    ...
    Searches.objects.filter(user=user, party=party).exclude(
        uri=result, user=user, party=party
    ).delete()
    current = Searches.objects.filter(uri=result, user=user, party=party).first()
    search_text = request.GET.get("text", None)
    ...
    for track in tracks:
        ...
        track_name = (track_name[:255] + "..") if len(track_name) > 255 else track_name
...
def settings(request, pid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if "kill" in request.POST:
                ...
                party.save()
                return HttpResponseRedirect(reverse("start_party", kwargs={"pid": pid}))
    else:
        form = settingsForm(
            partyObject=party,
            initial={
    ...
    context = {"party": party, "form": form, "device": device}
        ...
        if form.is_valid():
            ...
            if "skip" in request.POST:
                if party.roundNum == party.roundTotal:
                    category = Category.objects.filter(
                        party=party, roundNum=party.roundNum
                    ).first()
                    category.full = True
                    category.save()
                    reset_users(party)
                    ...
                    lead = Users.objects.filter(
                        party=party, turn="picking", active=True
                    ).first()
                    if lead:
                        lead.turn = "picked"
                        lead.save()
